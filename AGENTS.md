...
1. Search for open PRs that may already address the issue: `gh pr list --repo singi-labs/barazo-deploy --state open`
2. Search for related branches: `gh api repos/singi-labs/barazo-deploy/branches --paginate`
...
git worktree add /tmp/barazo-deploy-<branch-name> -b <branch-name> origin/main

# Work in the worktree
cd /tmp/barazo-deploy-<branch-name>

# When done, remove the worktree
git worktree remove /tmp/barazo-deploy-<branch-name>
```

Never work directly in the main checkout (`/singi-labs/repos/barazo-deploy/`). Always create a worktree per issue. Clean up the worktree after the PR is merged.
